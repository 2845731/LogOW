...
from bert_pytorch import Predictor, Trainer, BGL_Predictor
...
if __name__ == "__main__":
    ...
    predict_parser = subparsers.add_parser('multi_predict')
    predict_parser.set_defaults(mode='multi_predict')
    predict_parser.add_argument("-m", "--mean", type=float, default=0)
    predict_parser.add_argument("-s", "--std", type=float, default=1)
    ...
    #####################################
    #####################################
    # 为不进行dynamic_mask,所以预测过程的原理类似于LogBert,保留此部分代码用于对照实验
    # Predictor(options).predict()
    # Predictor(options).predict()

    # BGL_Predictor(options).predict()/multi-predict with uncertainty
    # 采用不确定性多次预测，此项目进行3次dynamic_mask,每次随机mask的设置不同的随机种子。
    BGL_Predictor(options).predict()
    #####################################
    #####################################
