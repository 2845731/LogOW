...
class LogDataset(Dataset):
    def __init__(self, log_corpus, time_corpus, vocab, seq_len, corpus_lines=None, encoding="utf-8", on_memory=True, predict_mode=False, mask_ratio=0.15, mask_seed=1):
        """
    ...
    def __getitem__(self, idx):
        ...
        k_masked, k_label, t_masked, t_label = self.random_item_seed_editon(k, t, self.mask_seed)
    ...
    def random_item_seed_editon(self, k, t, seed):
        tokens = list(k)
        output_label = []
        ...
        # 设置随机种子，保证每次的随机mask的结果可以复现--2023.5.4
        # ---------------------------------------------
        random.seed(seed)
        random_list = []

        for random_num in range(len(tokens)):
