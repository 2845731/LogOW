...
from bert_pytorch.dataset.sample import generate_train_valid, generate_train, generate_valid
...
class Trainer():
    def __init__(self, options):
        ...
        # self.train_file_name = options['train_file_name'] # 此时值为train_and_query_normal
        self.train_file_name = options['file_name']  # 此时值为train
        self.valid_file_name = options['valid_file_name']  # 此时值为valid
        self.query_file_name = options['query_file_name']  # 此时值为query
    ...
    def train(self):
        ...
        # logkey_train, logkey_valid, time_train, time_valid = generate_train_valid(self.output_path + self.train_file_name, window_size=self.window_size,
        ...
        if not self.go_on_pretrain:
            # 加载train
            logkey_train, time_train = generate_train(self.output_path + self.train_file_name,
            ...
            # 加载valid
            logkey_valid, time_valid = generate_valid(self.output_path + self.valid_file_name,
                                                      window_size=self.window_size,
                                                      adaptive_window=self.adaptive_window,
                                                      valid_size=self.valid_ratio,
                                                      sample_ratio=self.sample_ratio,
        ...
        if self.go_on_pretrain:
            # 加载query
            logkey_train, logkey_valid, time_train, time_valid = generate_train_valid(self.output_path + self.query_file_name,
                                                                                      window_size=self.window_size,
                                                                                      adaptive_window=self.adaptive_window,
                                                                                      valid_size=self.valid_ratio,
                                                                                      sample_ratio=self.sample_ratio,
        ...
        if self.go_on_pretrain:
            print("当前的self.go_on_pretrain为{}".format(self.go_on_pretrain))
            # print('建立全新的模型继续训练...')
            # bert = BERT(len(vocab), max_len=self.max_len, hidden=self.hidden, n_layers=self.layers,
            #             attn_heads=self.attn_heads,
            #             is_logkey=self.is_logkey, is_time=self.is_time)
            print('加载已有的模型继续训练...')
            bert_path = self.model_dir + "best_bert.pth"
            logbert_model = torch.load(bert_path)
            bert = logbert_model.bert
            mask_lm = logbert_model.mask_lm

        elif not self.go_on_pretrain:
            print("当前的self.go_on_pretrain为{}".format(self.go_on_pretrain))
            print('建立全新的模型继续训练...')
            bert = BERT(len(vocab), max_len=self.max_len, hidden=self.hidden, n_layers=self.layers,
                        attn_heads=self.attn_heads,
        ...
        if self.go_on_pretrain:
            self.trainer = BERTTrainer(bert, len(vocab), train_dataloader=self.train_data_loader,
                                       ...
                                       is_logkey=self.is_logkey, is_time=self.is_time,
                                       hypersphere_loss=self.hypersphere_loss, go_on_pretrain_index=True,
            ...
            self.trainer = BERTTrainer(bert, len(vocab), train_dataloader=self.train_data_loader,
                                       ...
                                       with_cuda=self.with_cuda, cuda_devices=self.cuda_devices, log_freq=self.log_freq,
                                       is_logkey=self.is_logkey, is_time=self.is_time,
                                       hypersphere_loss=self.hypersphere_loss, go_on_pretrain_index=False)
    ...
    def start_iteration(self, surfix_log):
        ...
        for epoch in range(self.epochs):
            ...
            if avg_loss < best_loss:
                ...
                elif self.go_on_pretrain:
                    self.trainer.save(self.refix_model_path)
                epochs_no_improve = 0
    ...
    def plot_train_valid_loss(self, surfix_log):
        ...
        plt.legend()
        if not self.go_on_pretrain:
            plt.savefig(self.model_dir + "train_valid_loss_go_on_pretrain.png")
        elif self.go_on_pretrain:
