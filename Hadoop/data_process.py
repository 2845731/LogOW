...
data_dir = os.path.expanduser(r"E:\logbert-main\datasets\hadoop")
# output_dir = "../output/bgl/"
output_dir = "../output/no_shuffle_hadoop/"
log_file = "hadoop.log"
...
def parse_log(input_dir, output_dir, log_file, parser_type):
    log_format = '<LineId> <Date> <Time> <Level> <Process> <Component> <Content> <EventId> <EventTemplate>'
    regex = [
...
if __name__ == "__main__":
    ...
    # Parser #
    #########
    if not os.path.exists(r"E:\logbert-main\output\hadoop\Hadoop_2k.log_structured.csv"):
    ...
    train_deeplog_df_len = int(deeplog_df_len * 0.8)
    train_deeplog_df = deeplog_df[:train_deeplog_df_len]
    test_deeplog_df = deeplog_df[train_deeplog_df_len:]

    #########
    # Train #
    #########
    train_df = train_deeplog_df[train_deeplog_df["Label"] == 0]
    # no shuffle
    train_len = int(len(train_df))
    # deeplog_file_generator(os.path.join(output_dir,'train'), train, ["EventId", "deltaT"])
    deeplog_file_generator(os.path.join(output_dir, 'train'), train_df, ["EventId"])
